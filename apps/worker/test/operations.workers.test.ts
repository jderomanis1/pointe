...
/**
 * Each test runs against a fresh DO instance — its constructor already ran
 * initSchema, so the schema is in place. Operations functions take `sql`
 * directly, so we hand them state.storage.sql via runInDurableObject. This
 * is the same API surface the tests had under the better-sqlite3 mock, but
 * the storage is now real DO SQLite.
 */
let counter = 0;
async function withSql<T>(fn: (sql: SqlStorage) => T | Promise<T>): Promise<T> {
  const name = `ops-${counter++}`;
  const stub = ROOM.get(ROOM.idFromName(name));
  // Wake the DO so the constructor runs (initSchema fires).
...
describe('operations (real DO SQLite)', () => {
  it('createRoom + getRoomState: room is lobby, host voter is connected with role host', async () => {
    await withSql((sql) => {
  ...
  it('createRoom twice throws ROOM_ALREADY_EXISTS', async () => {
    await withSql((sql) => {
      createRoom(sql, baseParams);
  ...
  it('addVoter persists a non-host voter visible in getRoomState', async () => {
    await withSql((sql) => {
      createRoom(sql, baseParams);
  ...
  it('getRoomState before createRoom throws ROOM_NOT_FOUND', async () => {
    await withSql((sql) => {
      expect(() => getRoomState(sql)).toThrow('ROOM_NOT_FOUND');
    });
  });

  it('round-trips customDeck through the custom_deck JSON column', async () => {
    await withSql((sql) => {
  ...
  it('addStory once: story has state=pending, orderIndex=100, edited=false, roomId set', async () => {
    await withSql((sql) => {
      createRoom(sql, baseParams);
  ...
  it('addStory three times: orderIndex is 100, 200, 300 in order', async () => {
    await withSql((sql) => {
      createRoom(sql, baseParams);
  ...
  it('editStory: changes text when no votes exist; edited stays false', async () => {
    await withSql((sql) => {
      createRoom(sql, baseParams);
  ...
  it('editStory: edited becomes true if a vote exists when text changes', async () => {
    await withSql((sql) => {
      createRoom(sql, baseParams);
      addStory(sql, { storyId: 's-1', text: 'old text', now: NOW + 1 });
      sql.exec(
  ...
  it('editStory: throws STORY_NOT_FOUND for unknown storyId', async () => {
    await withSql((sql) => {
      createRoom(sql, baseParams);
  ...
  it('openVoting: pending → active and sets openedAt; clearedVotes is null (first-open)', async () => {
    await withSql((sql) => {
      createRoom(sql, baseParams);
      addStory(sql, { storyId: 's-1', text: 'one', now: NOW + 1 });
  ...
  it('openVoting: throws ANOTHER_STORY_ACTIVE when another story is already active', async () => {
    await withSql((sql) => {
      createRoom(sql, baseParams);
      addStory(sql, { storyId: 's-1', text: 'one', now: NOW + 1 });
      addStory(sql, { storyId: 's-2', text: 'two', now: NOW + 2 });
  ...
  it('openVoting: throws STORY_NOT_OPENABLE when the story is in a non-openable state (active, committed)', async () => {
    await withSql((sql) => {
      createRoom(sql, baseParams);
      addStory(sql, { storyId: 's-1', text: 'one', now: NOW + 1 });
  ...
  it('openVoting: throws STORY_NOT_FOUND for missing storyId', async () => {
    await withSql((sql) => {
      createRoom(sql, baseParams);
  ...
  function seedActive(sql: SqlStorage) {
    createRoom(sql, baseParams);
    addStory(sql, { storyId: 's-1', text: 'one', now: NOW + 1 });
    openVoting(sql, { storyId: 's-1', now: NOW + 2 });
  }

  it('castVote: records the vote and it appears in getRoomState().votes', async () => {
    await withSql((sql) => {
      seedActive(sql);
  ...
  it('castVote: re-cast updates the same row; submittedAt stable, updatedAt advances', async () => {
    await withSql((sql) => {
      seedActive(sql);
      ...
      expect(second.submittedAt).toBe(NOW + 10);
      expect(second.updatedAt).toBe(NOW + 20);
  ...
  it('castVote: throws STORY_NOT_ACTIVE when the story is not active', async () => {
    await withSql((sql) => {
      createRoom(sql, baseParams);
      addStory(sql, { storyId: 's-1', text: 'one', now: NOW + 1 });
      expect(() =>
  ...
  it('castVote: throws SPECTATOR_CANNOT_VOTE when the voter is a spectator', async () => {
    await withSql((sql) => {
      seedActive(sql);
  ...
  it('castVote: throws VOTER_NOT_FOUND for unknown voterId', async () => {
    await withSql((sql) => {
      seedActive(sql);
      expect(() =>
  ...
  it('castVote: rejects confidence outside 1–5 with INVALID_CONFIDENCE', async () => {
    await withSql((sql) => {
      seedActive(sql);
      expect(() =>
  ...
  it('revealVotes: active → revealed; sets revealedAt; returns the votes', async () => {
    await withSql((sql) => {
      seedActive(sql);
      castVote(sql, { storyId: 's-1', voterId: 'host-1', points: '5', confidence: 3, now: NOW + 10 });
  ...
  it('revealVotes: throws STORY_NOT_ACTIVE on a non-active story', async () => {
    await withSql((sql) => {
      createRoom(sql, baseParams);
      addStory(sql, { storyId: 's-1', text: 'one', now: NOW + 1 });
  ...
  it('commitStory: revealed → committed and sets finalEstimate', async () => {
    await withSql((sql) => {
      seedActive(sql);
  ...
  it('commitStory: throws STORY_NOT_REVEALED on a non-revealed story', async () => {
    await withSql((sql) => {
      seedActive(sql);
      expect(() =>
  ...
  it('resumeOrAddVoter: new voter requires displayName; missing → DISPLAY_NAME_REQUIRED', async () => {
    await withSql((sql) => {
      createRoom(sql, baseParams);
      expect(() =>
  ...
  it('resumeOrAddVoter: new path inserts a fresh voter with the given role', async () => {
    await withSql((sql) => {
      createRoom(sql, baseParams);
  ...
  it('getHostVoterId: returns null before createRoom; returns the host id after', async () => {
    await withSql((sql) => {
  ...
  it('setVoterConnection: updates connection_state and last_seen_at; visible via getRoomState', async () => {
    await withSql((sql) => {
      createRoom(sql, baseParams);
  ...
  it('resumeOrAddVoter: resume reuses the existing voter id; keeps name/role; reactivates connection', async () => {
    await withSql((sql) => {
      createRoom(sql, baseParams);
      const v = resumeOrAddVoter(sql, {
      ...
      expect(v.displayName).toBe('Host');
      expect(v.connectionState).toBe('connected');
